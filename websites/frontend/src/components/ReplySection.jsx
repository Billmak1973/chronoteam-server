...
const ReplySection = ({
  ...
  const [localValidMentions, setLocalValidMentions] = useState(
    validMentionedUsers || [],
  );
  // ==========================================
  ...
  useEffect(() => {
    ...
    if (targetReviewId && replies.length > 0) {
      const isTargetInReplies = replies.some(
        (r) => r.id.toString() === targetReviewId,
      );
          ...
          setTimeout(() => {
            ...
            if (targetEl) {
              ...
              targetEl.style.transition =
                "box-shadow 0.5s, background-color 0.5s";
  ...
  const fetchReplies = useCallback(
    async (targetPage = 0, targetSort = sort) => {
      setLoading(true);
      try {
        const res = await fetch(
          `/api/review/${reviewId}/replies?page=${targetPage}&size=20&sort=${targetSort}`,
        ...
        if (result.success) {
          ...
          setSort(targetSort);
          // 【新增】合併樓中樓返回的合法用戶名
          if (result.data.validMentionedUsers) {
            setLocalValidMentions((prev) => {
              const merged = new Set([
                ...(prev || []),
                ...result.data.validMentionedUsers,
              ]);
              return [...merged];
            });
          }
    ...
    [reviewId, sort, validMentionedUsers],
  );
  ...
  const handleReaction = async (replyId, isDislike = false) => {
    ...
    const api = isDislike
      ? `/api/review/${replyId}/dislike`
      : `/api/review/${replyId}/like`;
    ...
    try {
      ...
      if (result.success) {
        ...
        if (
          result.message &&
          (result.message.includes("1分鐘內操作達到") ||
            result.message.includes("限制"))
        ) {
  ...
  /**
   * 將評論內容中的 @用戶名 渲染為藍色高亮
   * @param {string} content - 純文本內容
   * @param {Set|Array} validUsers - 後端返回的合法用戶名集合
   */
  const renderContentWithMentions = (content, validUsers) => {
    if (!content || !validUsers || validUsers.length === 0) {
      return content;
    }

    // 按長度降序排列，防止短用戶名誤匹配長用戶名的子串
    const sortedUsers = [...validUsers].sort((a, b) => b.length - a.length);
    // 構建正則：匹配 @用戶名（僅匹配後端確認存在的）
    const escapedUsers = sortedUsers.map((u) =>
      u.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),
    );
    const regex = new RegExp(`@(${escapedUsers.join("|")})`, "g");

    // 分割文本並替換
    const parts = content.split(regex);

    return parts.map((part, index) => {
      // 奇數索引是捕獲組（即用戶名），偶數索引是普通文本
      if (index % 2 === 1 && sortedUsers.includes(part)) {
        return (
          <span
            key={index}
            style={{
            ...
            onClick={() => {
              // 可選：點擊跳轉到該用戶主頁或觸發其他操作
              window.location.href = `/user/${encodeURIComponent(part)}`;
            }}
          >
            @{part}
          </span>
        ...
        <div className="reply-toolbar">
          <div className="reply-toolbar-left">
            {initialReplyCount > 0 && !isOpen && (
              ...
              >
                <i className="fas fa-comments"></i> 查看 {initialReplyCount}{" "}
                條回覆
              </button>
            )}
            {isOpen && (
          ...
          {isOpen && (
            <div className="reply-toolbar-right">
              <select
                onChange={(e) => fetchReplies(0, e.target.value)}
                value={sort}
        ...
        <div className="reply-list-container">
          ...
          {loading ? (
            <div className="reply-loading">
              <i className="fas fa-spinner fa-spin"></i> 加載中...
            </div>
          ) : (
                  ...
                  <div className="reply-item">
                    <div className="reply-header">
                      <div className="reply-user">
                        {/* 【修改 1】：將用戶名包裹在一個可點擊的 span 中，跳轉到 /user/username */}
                        <strong
                          style={{ cursor: "pointer" }}
                          onClick={() =>
                            (window.location.href = `/user/${encodeURIComponent(reply.customer.username)}`)
                          }
                          title={`查看 ${reply.customer.username} 的主頁`}
                        >
                        ...
                        {reply.customer.username === currentUsername && (
                          <span className="reply-badge">我</span>
                        )}

                        {/* 【修改 2】：如果被回復的用戶存在，也讓其名字可點擊 (可選，根據需求) */}
                        {reply.replyToUser && (
                          <span className="reply-to">
                            ...
                            <span
                              ...
                              onClick={() =>
                                (window.location.href = `/user/${encodeURIComponent(reply.replyToUser)}`)
                              }
                              title={`查看 ${reply.replyToUser} 的主頁`}
                            >
                              {reply.replyToUser}
                            </span>
                          </span>
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          if (
                            ...
                            if (
                              typeof window.showLoginRequiredModal ===
                              "function"
                            ) {
                              window.showLoginRequiredModal();
                            }
                      ...
                      {currentUsername &&
                        ...
                        blockedUsers &&
                          ...
                          <button
                            ...
                            onClick={() =>
                              handleUnblock(reply.customer.username)
                            }
                            title="解除禁言"
                          >
                          ...
                          <button
                            className="reply-action-btn btn-ban"
                            onClick={() =>
                              handleBlockClick(reply.customer.username)
                            }
                            title="雙向禁言"
                          >
                            <i className="fas fa-ban"></i> 禁言
          ...
          {totalPages > 1 && (
            <div className="reply-pagination">
              ...
              {Array.from({ length: totalPages }, (_, i) => (
                <button
                  key={i}
                  className={`btn ${i === page ? "btn-gold" : "btn-outline"}`}
                  onClick={() => fetchReplies(i)}
                >
                  {i + 1}
                </button>
              ))}
      ...
      {!isOpen && showReplyForm && replyParentId === null && (
        <div className="reply-form active" style={{ marginTop: "1rem" }}>
          ...
          <div className="form-actions">
            ...
            <button className="btn-submit-reply" onClick={handleSubmitReply}>
              提交
            </button>
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            <p style={{ marginBottom: "1rem", color: "var(--gray)" }}>
              請輸入禁言時長，用戶 <strong>{targetBlockUser}</strong>{" "}
              在期間內將無法在任何評論區回覆：
            </p>
            <div
              style={{
        ...
        >
          ...
          >
            ...
            <p style={{ marginBottom: "1rem", color: "var(--gray)" }}>
              請選擇刪除原因，這將發送通知給用戶{" "}
              <strong>
                {replies.find((r) => r.id === deletingReplyId)?.customer
                  ?.username || "該用戶"}
              </strong>
              ：
            </p>
