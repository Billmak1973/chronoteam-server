...
public class ReviewController {
    ...
    public ResponseEntity<ApiResponse> deleteReview(
        ...
        try {
            ...
            if ("admin".equals(username)) {
                ...
                notification.setRecipientUsername(targetUser);
    ...
    public ResponseEntity<ApiResponse> pinReview(
        ...
        try {
            ...
            if (pinned != null) {
                ...
                if (pinned) {
                    long pinnedCount = reviewRepository.countByProduct_IdAndPinned(review.getProduct().getId(), true);
                    if (pinnedCount >= 3) {
    ...
    /**
     * 獲取某個商品的根評論列表 (分頁 + 排序 + 樓中樓數量 + 當前用戶點贊狀態)
     */
    ...
    public ResponseEntity<?> getRootReviews(
            ...
            @RequestParam(defaultValue = "popular") String sort,
            Authentication authentication) { // 🟢 1. 注入 Authentication 獲取當前用戶
        ...
        // 2. 查詢根評論
        Page<Review> rootReviewsPage = reviewRepository.findByProduct_IdAndParentIdIsNull(productId, pageable);
        ...
        for (Review r : rootReviewsPage.getContent()) {
            ...
            map.put("createdAt", r.getCreatedAt());
            map.put("replyCount", replyCounts.getOrDefault(r.getId(), 0L));
            ...
            if (username != null) {
                ...
                var reactionOpt = reviewReactionRepository.findByReviewIdAndUsername(r.getId(), username);
